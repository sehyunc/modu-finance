...
import {useApprove, useOnboard, useRibbon} from "hooks/index";
...
export const VaultForm = ({ onClose, provider, tokenAddress, vaultAddress}) => {
  ...
  const buttonText = !isDeposit ? `Withdraw ${tokenName}` : (isApproved ?`Deposit ${tokenName}` : `Approve ${tokenName}`);
    ...
    <>
      <Box>
        ...
        >
          ...
          <InputGroup>        
          <Input value={inputText} onChange={event => setInputText(event.target.value)} mb="12" variant="filled" placeholder="0" size="lg" />
        ...
        <Button h="1.75rem" size="sm" onClick={()=>{
          ...
          <Button
            ...
            w="100%"
            mb="6"
            onClick={!onConnectToMetaMask ? closeAndConnect : async () => { 
              if(!isApproved){
                await approve();
              }else if(isDeposit){
